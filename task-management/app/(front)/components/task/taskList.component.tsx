...
export const TaskList = ({ tasks }: TaskSectionProp) => {
  const [taskCompleteOrWontDo, setTaskCompleteOrWontDo] = useState<ITaskModel[]>([])
  const [taskPending, setTaskPending] = useState<ITaskModel[]>([])
  const [openCollapsible, setOpenCollsapsible] = useState<boolean>(true)
  useEffect(() => {
...
const handleUpdateTaskStatus = (id: string, status: IStatus) => {
  ...
  return (
    <>
      {taskPending
        .map((value: ITaskModel) => (
          <div
            key={value.id}
            className="flex gap-2 items-center bg-white w-full mt-2 rounded-md border border-gray-200 shadow  "
          >
            <Task task={value} onDelete={deleteTask} onUpdateStatus={handleUpdateTaskStatus} depth={0} />
          </div>
        ))}
...
<Collapsible key={"completed-tasks"} open={openCollapsible} onOpenChange={(open) => setOpenCollsapsible(open)} className="mt-2 w-full">
        <CollapsibleTrigger className="w-full text-xs font-bold p-2 flex gap-2 items-center">
          <ChevronLeft className={`w-4 h-4 transition-all ${openCollapsible && "-rotate-90"}`} /><span className="flex items-center gap-1"> Hoàn thành/Không làm <span className="text-neutral-400 font-normal shrink-0 text-center">{taskCompleteOrWontDo.length}</span></span>
        </CollapsibleTrigger>
