...
interface EndDateProp {
  selectedDate: Date;
  setEndDate: (endDate: Date | null) => void;
  endDate: Date | null;
}
const EndDate = ({ selectedDate, setEndDate, endDate }: EndDateProp) => {
    ...
    >
      ...
      >
        <Button variant="outline" className="w-full justify-between">
          <span className="flex gap-2 items-center">
            <Calendar1 />{" "}
            <p>{endDate ? formatSpecific(endDate) : "Ngày kết thúc"}</p>
          </span>
      ...
      <DropdownMenuContent className="w-full z-60 h-full">
        <div className="flex gap-2 p-2 items-center justify-center border-b border-gray-300">
          <p className=" text-Center text-primary">Chọn ngày kết thúc</p>
        </div>
