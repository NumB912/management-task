...
import { DeleteTagOnlyMeUsecase, UpdateTagOnlyMeUsecase } from "@/app/core/application";
...
export async function DELETE(
    ...
    const getByID = await container
        .resolve<DeleteTagOnlyMeUsecase>(TYPES.DeleteTagOnlyMeUsecase)
        .execute({
            id: tagId,
            userId: user.id,
        });
...
export async function PUT(
    ...
    const updated = await (await GetContainer())
        .resolve<UpdateTagOnlyMeUsecase>(TYPES.UpdateTagOnlyMeUsecase)
        .execute({
            id: tagId,
            name: name,
            userId: user.id,
